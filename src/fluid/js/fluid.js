...
function handleMouseOut(e) {
  ...
  if (windowMovementInterval == -1 && e.toElement == null && e.relatedTarget == null) {
...
function addGUI() {
  gui
    .add(fluidParams, "NumParticles", 0, 5000)
    .step(10)
...
function doLoop() {
  engine.doPhysics();
  for (var i = 0; i < meshes.length; i++) engine.getParticlePosition(i, meshes[i].position);
  renderer.render(scene, camera);
